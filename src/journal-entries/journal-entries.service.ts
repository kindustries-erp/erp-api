...
export class JournalEntriesService {
  ...
  async reverse(id: string, dto: ReverseJournalEntryDto, userToken: string) {
    this.guard(userToken);

    // Load original entry + lines
    const entryRes = await fetch(
      `${this.directusUrl}/items/${this.collection}/${id}?fields[]=*`,
      { headers: this.getUserHeaders(userToken) },
    );
    if (entryRes.status === 404)
      throw new NotFoundException(`Không tìm thấy bút toán: ${id}`);
    ...
    const entryJson = await entryRes.json();
    const original = entryJson.data;

    if (original.status !== 'posted') {
      throw new BadRequestException(
        `Chỉ có thể đảo bút toán đã hạch toán. Trạng thái hiện tại: ${original.status}`,
      );
    }

    // Load lines
    const linesRes = await fetch(
      `${this.directusUrl}/items/${this.linesCollection}?filter=${encodeURIComponent(JSON.stringify({ journal_entry_id: { _eq: id } }))}&fields[]=account_id,debit,credit,description,sort`,
      { headers: this.getAdminHeaders() },
    );
    if (!linesRes.ok)
      await throwDirectusResponseError(
        linesRes,
        'Không thể tải dòng bút toán gốc',
      );
    const linesJson = await linesRes.json();
    const originalLines: any[] = linesJson.data || [];

    const reverseDate =
      dto.reverse_date || new Date().toISOString().slice(0, 10);
    const reversedVoucherNo = `REV-${original.voucher_no}`;
    ...
    const revHeaderRes = await fetch(
      `${this.directusUrl}/items/${this.collection}`,
      {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          voucher_no: reversedVoucherNo,
          date: reverseDate,
          period_id: original.period_id,
          description: dto.reason
            ? `[Đảo bút toán ${original.voucher_no}] ${dto.reason}`
            : `Đảo bút toán ${original.voucher_no}`,
          status: 'posted',
    ...
    if (!revHeaderRes.ok) {
      await throwDirectusResponseError(
        revHeaderRes,
        'Không thể tạo bút toán đảo',
      );
    }
    const revHeaderJson = await revHeaderRes.json();
    const reversedId = revHeaderJson.data?.id;

    // Create reversed lines (swap debit/credit)
    const revLines = originalLines.map((l: any, idx: number) => ({
      journal_entry_id: reversedId,
      account_id:
        typeof l.account_id === 'object' ? l.account_id.id : l.account_id,
      debit: l.credit,
      credit: l.debit,
    ...
    const revLinesRes = await fetch(
      `${this.directusUrl}/items/${this.linesCollection}`,
      {
        ...
        body: JSON.stringify(revLines),
      },
    );

    if (!revLinesRes.ok) {
      await fetch(
        `${this.directusUrl}/items/${this.collection}/${reversedId}`,
        {
          method: 'DELETE',
          headers: this.getAdminHeaders(),
        },
      ...
      await throwDirectusResponseError(
        revLinesRes,
        'Không thể tạo dòng bút toán đảo (đã rollback)',
      );
    }
    ...
    return {
      message: 'Đảo bút toán thành công',
      data: {
        original_id: id,
        reversed_entry_id: reversedId,
