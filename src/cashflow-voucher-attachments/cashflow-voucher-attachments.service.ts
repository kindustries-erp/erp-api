...
export class CashflowVoucherAttachmentsService {
  ...
  async create(dto: CreateCashflowVoucherAttachmentsDto, userToken: string) {
    ...
    try {
      ...
      return {
        message: 'Tạo đính kèm phiếu thu chi dòng tiền thành công',
        data: result,
      };
    } catch (error: any) {
      this.logger.error('Lỗi khi tạo đính kèm phiếu thu chi dòng tiền', error);
      const msg = error?.errors?.[0]?.message || error.message;
      throw new BadRequestException(`Lỗi: ${msg}`);
    }
  }

  async findAll(query: GetCashflowVoucherAttachmentsDto, userToken: string) {
    this.guard(userToken);
    const client = this.getClient(userToken);
    try {
      ...
      const filter: any = {};
      if (query.cashflow_voucher_id) {
        filter.cashflow_voucher_id = { _eq: query.cashflow_voucher_id };
      }
      if (query.search) {
        filter._or = [
      ...
      this.logger.error(
        'Lỗi khi lấy danh sách đính kèm phiếu thu chi dòng tiền',
        error,
      );
      throw new InternalServerErrorException(
        'Không thể lấy danh sách đính kèm phiếu thu chi dòng tiền',
      );
    }
  }
  ...
  async findOne(id: string, userToken: string) {
    ...
    try {
      ...
      return {
        message: 'Lấy thông tin đính kèm phiếu thu chi dòng tiền thành công',
        data: result,
      };
    } catch (error: any) {
      this.logger.error(
        `Lỗi khi lấy thông tin đính kèm phiếu thu chi dòng tiền ${id}`,
        error,
      );
      throw new InternalServerErrorException(
        'Không thể lấy thông tin đính kèm phiếu thu chi dòng tiền',
      );
    }
  }
  ...
  async update(
    id: string,
    dto: UpdateCashflowVoucherAttachmentsDto,
    userToken: string,
  ) {
    this.guard(userToken);
    const client = this.getClient(userToken);
    try {
      ...
      return {
        message: 'Cập nhật đính kèm phiếu thu chi dòng tiền thành công',
        data: result,
      };
    } catch (error: any) {
      this.logger.error(
        `Lỗi khi cập nhật đính kèm phiếu thu chi dòng tiền ${id}`,
        error,
      );
      const msg = error?.errors?.[0]?.message || error.message;
      throw new BadRequestException(`Lỗi: ${msg}`);
  ...
  async remove(id: string, userToken: string) {
    ...
    try {
      ...
      return { message: 'Xóa đính kèm phiếu thu chi dòng tiền thành công' };
    } catch (error: any) {
      this.logger.error(
        `Lỗi khi xóa đính kèm phiếu thu chi dòng tiền ${id}`,
        error,
      );
      throw new InternalServerErrorException(
        'Không thể xóa đính kèm phiếu thu chi dòng tiền',
      );
    }
  }
}
