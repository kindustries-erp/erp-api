...
export class ArWorkbenchController {
  ...
  @Get('documents')
  findDocuments(@Query() query: ArWorkbenchQueryDto, @UserToken() token: string) {
  ...
  @Patch('documents/:id')
  updateDocument(@Param('id') id: string, @Body() dto: UpdateArDocumentDto, @UserToken() token: string) {
    return this.service.updateDocument(id, dto, token);
  }

  @Get('applications')
  findApplications(@Query() query: ArWorkbenchQueryDto, @UserToken() token: string) {
    return this.service.findApplications(query, token);
  }

  @Post('applications')
  createApplication(@Body() dto: CreateArApplicationDto, @UserToken() token: string) {
    return this.service.createApplication(dto, token);
  }

  @Get('collection-activities')
  findCollectionActivities(@Query() query: ArWorkbenchQueryDto, @UserToken() token: string) {
    return this.service.findCollectionActivities(query, token);
  }

  @Post('collection-activities')
  createCollectionActivity(@Body() dto: CreateArCollectionActivityDto, @UserToken() token: string) {
