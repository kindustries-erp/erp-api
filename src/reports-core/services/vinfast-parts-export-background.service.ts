...
export type VinfastPartsExportQuery = {
  dateFrom?: string;
  dateTo?: string;
  search?: string;
  sortBy?: string;
...
export type VinfastPartsExportProgressEvent = {
...
export type VinfastPartsExportHistoryItem = {
  jobId: string;
  fileName: string;
...
export type VinfastPartsExportHistoryResult = {
...
@Injectable()
export class VinfastPartsExportBackgroundService implements OnModuleDestroy {
  private static readonly HISTORY_TTL_MS = 24 * 60 * 60 * 1000;

  public readonly progress$ = new Subject<VinfastPartsExportProgressEvent>();

  private readonly jobs = new Map<string, VinfastPartsExportJob>();
  ...
  async startBackgroundExport(
    ...
    const job: VinfastPartsExportJob = {
      id: randomUUID(),
      userId,
      status: 'RUNNING',
      current: 0,
  ...
  getJobSnapshotForUser(
    userId: string,
  ): VinfastPartsExportProgressEvent | null {
    ...
    return {
      processId: 'vinfast-parts-xlsx-export',
      userId: job.userId,
      jobId: job.id,
      current: job.current,
      total: job.total,
  ...
  private async executeJob(
    ...
    try {
      ...
      doneJob.expiresAt =
        Date.now() + VinfastPartsExportBackgroundService.HISTORY_TTL_MS;
  ...
  private cleanupExpiredJobs() {
    const now = Date.now();
    const ttlMs = VinfastPartsExportBackgroundService.HISTORY_TTL_MS;
  ...
  private buildQueryFingerprint(query: VinfastPartsExportQuery) {
    const normalized = {
      dateFrom: query.dateFrom || '',
      dateTo: query.dateTo || '',
